...
public class ArbolBinarioOrdenado<T extends Comparable<T>>
    ...
    @Override public void elimina(T elemento)
    {
        ...
        Vertice vv = intercambiaEliminable(v);
        v.elemento = vv.elemento;
        Vertice padre;
        v = vv;
        padre = v.padre;
        if (v.hayIzquierdo())
    ...
    {
        Vertice verticea = vertice;
        Vertice nvertice = vertice(nuevoVertice(null));
        if (!verticea.hayIzquierdo() && verticea.hayDerecho())
        {
            nvertice = verticea.padre;
            Vertice derecho = verticea.derecho;
            if (nvertice != null)
            {
                if (nvertice.izquierdo == verticea)
                    nvertice.izquierdo = derecho;
                else
                    nvertice.derecho = derecho;
            }
            else
                raiz = derecho;
            derecho.padre = nvertice;
            return;
        }
        if (verticea.hayIzquierdo() && !verticea.hayDerecho())
        {
            nvertice = verticea.padre;
            Vertice n = verticea.izquierdo;
            if (nvertice != null)
            {
                if (nvertice.izquierdo == verticea)
                    nvertice.izquierdo = n;
                else
                    nvertice.derecho = n;
            }
            else
        ...
        if (!verticea.hayIzquierdo() && !verticea.hayDerecho())
        {
            ...
            if (nvertice.izquierdo == verticea)
                nvertice.izquierdo = null;
            if (nvertice.derecho == verticea)  
                nvertice.derecho = null;
            return;
            }
    }
